...
export const idValidationPost = param("id").custom((value) => {
...
export const idValidationPostID = param("postId").custom((value) => {
  if (!ObjectId.isValid(value)) {
    throw new Error("PostID not type ObjectId post");
  }
  return true;
});
